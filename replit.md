...
- **Jae chat nudge**: `maybeInjectReassessmentNudge()` in `server/routes.ts` runs on every `GET /api/users/:userId/messages`. If `notifyAssessmentReminder` is on, cadence > 0, and days since the last assessment exceeds the cadence window, it inserts a Jae message ("It's been about N days since your last check-in. Want to take a fresh assessment...") into the thread — capped to once per 24h via `users.lastAssessmentReminderSentAt`.
